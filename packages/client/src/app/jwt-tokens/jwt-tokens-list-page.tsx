...
export default function JWTTokensListPage() {
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * ROWS_PER_PAGE - tokens.length) : 0;
    ...
    <>
      ...
      <div className="flex flex-wrap gap-8">
        <Table className="w-full">
          <TableCaption>JWT Tokens</TableCaption>
          <TableHeader>
            <TableRow>
            ...
            {tokens
              ...
              .map((token) => (
                <TableRow key={token.id}>
                  <TableCell className={token.disabled ? "italic" : ""}>
                    {token.projectName}
                  </TableCell>
                  <TableCell className={token.disabled ? "italic" : ""}>
                    {token.creatorName}
                  ...
                  <TableCell
                    className={`text-sm opacity-70 mb-2 ${token.disabled ? "italic" : ""}`}
                  >
