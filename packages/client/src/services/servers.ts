...
> {
  ...
  const data: endpoints_schemas.MCPServerOptionsList = await response.json();
...
export async function getServerDetail(id: string) {
  ...
  return safeParseWithErrors(
    data,
    endpoints_schemas.MCPServerOptionsDetailSchema
  );
}

// Create a new server
export async function createServer(
  serverOptions: endpoints_schemas.MCPServerOptionsCreate
) {
